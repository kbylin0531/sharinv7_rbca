...
class EcvvCategory extends B2BCategory {
    ...
    protected function requestCategory($catID,$level){
        ...
        $result = self::myXml2Array($content);
        if((isset($result['li']))){
            $rst = [];
            foreach ($result['li'] as $item) {
                $rst[] = [
                    'name'  => $item['name'],
                    'id'  => $item['attrs']["categoryId"],
                    'hasChild'  => !empty($item['attrs']['IsLeaf']) ? ('false' === $item['attrs']['IsLeaf']) : false,
                ];
            }
            return $rst;
        }
     ...
     * @return array
     */
    protected static function myXml2Array($xml){
